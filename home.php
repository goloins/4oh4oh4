...
for($index = 0; $index < count($build_public_timeline_preview); $index++) {
	...
	echo '<td class="thumb"><a href="/user/' . htmlspecialchars($user['username']) . '"><img alt="' . htmlspecialchars($user['displayname']) . '\'s Avatar" src="' . htmlspecialchars($user['avatar_url']) . '"/></a></td>';
	echo '<td><strong><a href="/user/' . htmlspecialchars($user['username']) . '" title="User ' . htmlspecialchars($user['displayname']) . '">' . htmlspecialchars($user['displayname']) . '</a></strong>';
	echo '<p>' . htmlspecialchars($post['content']) . '</p>';
	echo '<span class="meta"><a href="/status/' . htmlspecialchars($post['id']) . '">' . format_time_ago($post['created_at']) . '</a> from web</span>';
	echo '</td></tr>';
}
